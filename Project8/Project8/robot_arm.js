...
var finger1 = 0, finger2 = 0, finger3 = 0, thumb = 0;
var finger4 = 0, finger5 = 0, finger6 = 0;
//----------------------------------------------------------------------------
// Define Shape Data 
//----------------------------------------------------------------------------
...
for (var i =0; i < wireCubeLookups.length; i++)
{
   ...
   shapes.wireCube.colors.push(red);
}
...
function render() {
   ...
   mv = mult(mv, translate(0.6, 0.1, 0.0));
   mv = mult(mv, scale(0.2, 0.2, 1.0));
   gl.uniformMatrix4fv(mvLoc, gl.FALSE, flatten(transpose(mv)));
   gl.drawArrays(armShape.type, armShape.start, armShape.size);
   mv = matStack.pop();
   ...
   mv = mult(mv, translate(0.3, 0.3, 0.0));
   mv = mult(mv, scale(0.2, 0.2, 1.0));
   gl.uniformMatrix4fv(mvLoc, gl.FALSE, flatten(transpose(mv)));
   gl.drawArrays(armShape.type, armShape.start, armShape.size);
   mv = matStack.pop();
   ...
   mv = mult(mv, translate(0.0, 0.1, 0.0));
   mv = mult(mv, scale(0.2, 0.2, 1.0));
   gl.uniformMatrix4fv(mvLoc, gl.FALSE, flatten(transpose(mv)));
   gl.drawArrays(armShape.type, armShape.start, armShape.size);
   mv = matStack.pop();
   ...
   matStack.push(mv);
   mv = mult(mv, rotate(finger4, vec3(0, 0, 1)));
   mv = mult(mv, translate(0.6, -0.1, 0.0)); // Adjust translation for finger4
   mv = mult(mv, scale(0.2, 0.2, 1.0));
   gl.uniformMatrix4fv(mvLoc, gl.FALSE, flatten(transpose(mv)));
   gl.drawArrays(armShape.type, armShape.start, armShape.size);
   mv = matStack.pop();
   ...
   matStack.push(mv);
   mv = mult(mv, rotate(finger5, vec3(0, 0, 1)));
   mv = mult(mv, translate(0.3, -0.3, 0.0)); // Adjust translation for finger5
   mv = mult(mv, scale(0.2, 0.2, 1.0));
   gl.uniformMatrix4fv(mvLoc, gl.FALSE, flatten(transpose(mv)));
   gl.drawArrays(armShape.type, armShape.start, armShape.size);
   mv = matStack.pop();
   ...
   matStack.push(mv);
   mv = mult(mv, rotate(finger6, vec3(0, 0, 1)));
   mv = mult(mv, translate(0.0, -0.1, 0.0)); // Adjust translation for finger6
   mv = mult(mv, scale(0.2, 0.2, 1.0));
   gl.uniformMatrix4fv(mvLoc, gl.FALSE, flatten(transpose(mv)));
   gl.drawArrays(armShape.type, armShape.start, armShape.size);
   mv = matStack.pop();
...
function handleKeys(timePassed) 
{
  ...
  if (shift && isPressed("F")) {
      if (finger1 < 45) finger4 += d;
      else finger1 = 45;
      if (finger2 < 45) finger5 += d;
      else finger2 = 45;
      if (finger3 < 45) finger6 += d;
      else finger3 = 45;
  }
  if (!shift && isPressed("F")) {
      if (finger1 > -45) finger4 -= d;
  ...
  if (shift && isPressed("X")) {
   // Rotate arm on X axis with positive direction
   mv = mult(mv, rotate(d, vec3(1, 0, 0)));
}
if (!shift && isPressed("x")) {
   // Rotate arm on X axis with negative direction
   mv = mult(mv, rotate(-d, vec3(1, 0, 0)));
}

if (shift && isPressed("Y")) {
   // Rotate arm on Y axis with positive direction
   mv = mult(mv, rotate(d, vec3(0, 1, 0)));
}
if (!shift && isPressed("y")) {
   // Rotate arm on Y axis with negative direction
   mv = mult(mv, rotate(-d, vec3(0, 1, 0)));
}

// Finger Rotation Updates
if (shift && isPressed("A")) {
   // Rotate fingers on x-axis with positive direction
   finger1 += d;
   finger2 += d;
   finger3 += d;
}
if (!shift && isPressed("a")) {
   // Rotate fingers on x-axis with negative direction
   finger1 -= d;
   finger2 -= d;
   finger3 -= d;
}
...
if (shift && isPressed("M")) {
   // Rotate fingers on y-axis with positive direction
   finger1 += d;
   finger2 += d;
   finger3 += d;
}
...
if (!shift && isPressed("m")) {
   // Rotate fingers on y-axis with negative direction
   finger1 -= d;
   finger2 -= d;
   finger3 -= d;
}

// Toggle between solid and wire cubes
if (isPressed("T")) {
   shapes.wireCube.type = (shapes.wireCube.type === gl.LINE_STRIP) ? gl.TRIANGLES : gl.LINE_STRIP;
   shapes.solidCube.type = (shapes.solidCube.type === gl.TRIANGLES) ? gl.LINE_STRIP : gl.TRIANGLES;
}

// Toggle between perspective and ortho projections
if (isPressed("P")) {
   p = (p === perspective) ? ortho(-3.4 * aspect, 3.4 * aspect, -3.4, 3.4, 1.0, 20.0) : perspective(40.0, aspect, 0.1, 100.0);
   gl.uniformMatrix4fv(projLoc, gl.FALSE, flatten(transpose(p)));
}
}
